...
class LinearAccInteractivePlot:
    """
    Interactive playback widget for Linear Acceleration data (X, Y & Z axes)
    ...
    def __init__(self, all_data):
        ...
        # compute overall time min/max from Linear Acceleration.csv
    ...
    def _compute_time_range(self):
        ...
        if not times:
            raise RuntimeError("No time data found in any Linear Acceleration.csv")
    ...
    def _plot(self, datasets, axis, time_range):
        ...
        # map axis to column name
        col_map = {
            'X': 'Linear_Acceleration_x_m/s^2',
            'Y': 'Linear_Acceleration_y_m/s^2',
            'Z': 'Linear_Acceleration_z_m/s^2'
        }
        colname = col_map[axis]

        records = []
        for ds in datasets:
            df = self.all_data[ds].get('Linear Acceleration.csv')
        ...
        if not records:
            plt.text(0.5, 0.5, "No linear-acceleration data for those selections",
                     ha='center')
            plt.axis('off')
            display(plt.gcf())
            return
        ...
        plt.xlabel('Time (s)')
        plt.ylabel(f"Linear accel {axis} (m/s²)")
        plt.title(f"Linear acceleration {axis} over {start:.1f}–{end:.1f}s")
...
# Example usage:
# plotter = LinearAccInteractivePlot(all_data)
# plotter.display()
